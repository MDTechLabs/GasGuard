...
export class IncrementalCacheService {
  ...
  async getCachedAnalysis(
    repoPath: string,
  ): Promise<Map<string, AnalysisCacheEntry>> {
    const cacheKey = `analysis-cache:${this.normalizeRepoPath(repoPath)}`;
    const cached =
      await this.cacheService.get<Record<string, AnalysisCacheEntry>>(cacheKey);
  ...
  async getCachedDependencyGraph(repoPath: string): Promise<DependencyGraph> {
    ...
    return {
      nodes: new Map(
        Object.entries(cached.nodes).map(([k, v]) => [k, new Set(v)]),
      ),
      reverseNodes: new Map(
        Object.entries(cached.reverseNodes).map(([k, v]) => [k, new Set(v)]),
      ),
    };
  }
  ...
  async cacheDependencyGraph(
    ...
    const serializableGraph = {
      nodes: Object.fromEntries(
        Array.from(graph.nodes.entries()).map(([k, v]) => [k, Array.from(v)]),
      ),
      reverseNodes: Object.fromEntries(
        Array.from(graph.reverseNodes.entries()).map(([k, v]) => [
          k,
          Array.from(v),
        ]),
      ),
  ...
  async performIncrementalAnalysis(
    ...
    // Get hash comparison
    const hashComparison = await this.fileHashService.compareWithCache(
      repoPath,
      supportedFiles,
    );
    ...
    // Add modified files
    hashComparison.modified.forEach((file) =>
      filesToReanalyze.add(file.filePath),
    );

    // Add files that depend on modified files
    for (const modifiedFile of hashComparison.modified) {
      const dependents = dependencyGraph.reverseNodes.get(
    ...
    const filesToAnalyze = Array.from(filesToReanalyze).filter((filePath) => {
      ...
      const currentFile = hashComparison.unchanged.find(
    ...
    for (const result of newResults) {
      const fileInfo =
        hashComparison.modified.find((f) => f.filePath === result.source) ||
        hashComparison.added.find((f) => f.filePath === result.source);

      if (fileInfo) {
        // Build dependency graph for this file
        const dependencies = await this.fileHashService.findDependentFiles(
          [fileInfo.filePath],
          supportedFiles,
        );
    ...
    [
      ...hashComparison.modified,
      ...hashComparison.added,
  ...
  async getCacheStats(repoPath: string): Promise<{
    ...
    if (cachedAnalysis.size > 0) {
      const oldestEntry = Array.from(cachedAnalysis.values()).reduce(
        (oldest, current) =>
   ...
   */
  async shouldUseIncrementalAnalysis(
    repoPath: string,
    fileCount: number,
  ): Promise<boolean> {
    ...
    if (stats.cacheAge && stats.cacheAge > 7 * 24 * 60 * 60 * 1000) {
      // 7 days
      return false;
    }
