...
export class AnalysisValidator extends BaseValidator {
  ...
  private static readonly SUPPORTED_FRAMEWORKS = [
    "soroban",
    "solidity",
    "general",
  ];
    ...
    try {
      ...
      if (!body.project) {
        ...
        if (
          ...
          !this.isValidUrl(body.project.repositoryUrl)
        ) {
          errors.push({
      ...
      if (
        ...
        body.files.forEach((file, index) => {
          ...
          if (
            !file.language ||
            !this.SUPPORTED_LANGUAGES.includes(file.language)
          ) {
            errors.push({
      ...
      if (body.options) {
        ...
        if (
          !["low", "medium", "high", "critical"].includes(body.options.severity)
        ) {
          errors.push({
      ...
      if (body.metadata) {
        if (
          body.metadata.framework &&
          !this.SUPPORTED_FRAMEWORKS.includes(body.metadata.framework)
        ) {
          errors.push({
  ...
  private static validateSorobanProject(
    files: any[],
    metadata: any,
    ...
    if (cargoToml) {
      ...
      const hasSorobanSdk =
        content.includes("soroban-sdk") || content.includes("stellar-sdk");

      if (!hasSorobanSdk) {
        errors.push({
          field: "metadata.dependencies",
          message:
            "Soroban projects must include soroban-sdk or stellar-sdk dependency",
    ...
    const contractFiles = rustFiles.filter(
      (f) =>
    ...
    if (contractFiles.length === 0) {
      errors.push({
        field: "files",
        message:
          "No Soroban contract implementation found. Files should contain #[contractimpl] or contract definitions.",
