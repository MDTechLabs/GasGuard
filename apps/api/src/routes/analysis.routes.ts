...
export function createAnalysisRoutes(queue: any): Router {
  ...
  router.post("/analysis", AnalysisValidator.validateSubmission, (req, res) =>
    controller.submitCodebase(req, res),
  );
  ...
  // Get analysis result
  router.get("/analysis/:id/result", (req, res) =>
    controller.getAnalysisResult(req, res),
  );

  // Cancel analysis
  router.delete("/analysis/:id", (req, res) =>
    controller.cancelAnalysis(req, res),
  );
