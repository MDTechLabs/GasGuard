  ...
  .action(
    async (
      ...
      try {
        ...
        console.error(
          chalk.red(`✖ Failed to read file: ${(err as Error).message}`),
        );
        process.exit(1);
      }
      ...
      try {
        ...
        console.error(
          chalk.red(`✖ Failed to parse AST: ${(err as Error).message}`),
        );
        process.exit(1);
      }
      ...
      if (options.stats) {
        ...
        console.log(
          chalk.blue(
            `AST stats for ${path.basename(absPath)} [${snapshot.ast.language}]`,
          ),
        );
