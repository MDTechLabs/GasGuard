...
export function printSummary(
  ...
  console.log(
    `  Total Violations: ${chalk.yellow(results.summary.totalViolations.toString())}`,
  );
  console.log(
    `  Total Gas Savings: ${chalk.green(formatGasSavings(results.summary.totalGasSavings))}\n`,
  );
  ...
  if (results.findings.length > 0) {
    ...
    for (const finding of results.findings.slice(0, 20)) {
      // Show first 20
...
function printFinding(finding: any, options: SummaryOptions): void {
  ...
  if (finding.gasSavings) {
    console.log(
      chalk.green(`  Gas Savings: ${formatGasSavings(finding.gasSavings)}`),
    );
  }
...
export function printCompactSummary(results: ScanResult): void {
  ...
  console.log(
    `${status} ${results.summary.totalViolations} issues found (${critical} critical, ${high} high, ${medium} medium, ${low} low) - ${formatGasSavings(results.summary.totalGasSavings)} potential savings`,
  );
}
