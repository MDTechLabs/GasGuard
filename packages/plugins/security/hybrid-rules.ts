...
abstract class HybridRule {
  ...
  protected createHybridIssue(
    ...
    return {
      ...
      suggestedFix: {
        description:
          gasOptimization || "Review and fix this security vulnerability",
      },
      gasImpact,
      gasOptimization,
...
class UncheckedExternalCallRule extends HybridRule {
  ...
  description =
    "Detects unchecked external calls that may lead to failed transactions and wasted gas";
  ...
  async check(ast: any, sourceCode: string): Promise<HybridRuleResult> {
    ...
    const uncheckedCallPattern =
      /\.(call|delegatecall|staticcall|transfer|send)\([^)]*\)/g;
    let match;
...
class IntegerOverflowRule extends HybridRule {
  ...
  description =
    "Detects potential integer overflow/underflow that can cause security issues and unexpected gas consumption";
  ...
  async check(ast: any, sourceCode: string): Promise<HybridRuleResult> {
    ...
    const unsafeMathPattern =
      /(?:\w+\s*[\+\-\*\/]\s*\w+|\w+\s*\+=\s*\w+|\w+\s*-=\s*\w+)/g;
    let match;
    ...
    while ((match = unsafeMathPattern.exec(sourceCode)) !== null) {
      ...
      if (
        ...
        !sourceCode.includes("using SafeMath")
      ) {
        issues.push(
          this.createHybridIssue(
        ...
        gasIssues.push({
          ...
          description:
            "Overflow conditions can cause transaction reversals and gas waste",
          line,
          gasAmount: 5000,
...
class ReentrancyGasRule extends HybridRule {
  ...
  description =
    "Detects reentrancy vulnerabilities that can lead to gas drain attacks";
  ...
  async check(ast: any, sourceCode: string): Promise<HybridRuleResult> {
    ...
    const callBeforeStateChangePattern =
      /(\w+\.(call|transfer|send)\([^)]*\)[\s\S]*?)(\w+\s*=)/g;
    let match;
