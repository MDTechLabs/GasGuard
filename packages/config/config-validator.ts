...
export class ConfigValidator {
  ...
  private validateSystemConfig(
    ...
    if (
      !["development", "staging", "production"].includes(system.environment)
    ) {
      errors.push({
    ...
    if (system.performance) {
      if (
        typeof system.performance.maxConcurrency !== "number" ||
        system.performance.maxConcurrency < 1
      ) {
        errors.push({
      ...
      if (
        typeof system.performance.timeoutMs !== "number" ||
        system.performance.timeoutMs < 0
      ) {
        errors.push({
    ...
    if (system.security) {
      ...
      if (
        typeof system.security.maxRequestsPerMinute !== "number" ||
        system.security.maxRequestsPerMinute < 1
      ) {
        errors.push({
      ...
      if (!rule.id) {
        ...
        if (!this.isValidRuleId(rule.id)) {
          errors.push({
            path: `${prefix}.id`,
            message:
              "Invalid rule ID format (should be lowercase, alphanumeric, hyphens only)",
  ...
  private validateAction(
    ...
    if (
      action.severity &&
      !["critical", "high", "medium", "low", "info"].includes(action.severity)
    ) {
      errors.push({
        path: `${path}.severity`,
