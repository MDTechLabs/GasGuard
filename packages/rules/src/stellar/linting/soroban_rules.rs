    ...
    fn check(&self, source: &str, file_path: &str) -> Option<Vec<RuleViolation>> {
        ...
        if !source.contains("soroban_sdk") {
            violations.push(RuleViolation {
                ...
                suggestion: "Add `use soroban_sdk::{contract, contractimpl, contracttype};`".to_string(),
                line_number: 1,
                column_number: 0,
        ...
        if source.contains("#[contractimpl]") && !source.contains("#[contracttype]") {
            violations.push(RuleViolation {
                rule_name: self.id().to_string(),
                description: "Contract uses #[contractimpl] but missing #[contracttype]".to_string(),
            ...
            if line.contains("pub fn") && !line.contains("Env") {
                ...
                if i + 1 < lines.len() {
                    ...
                    if next_lines.contains(".set(") || next_lines.contains(".put(") {
                        violations.push(RuleViolation {
                            rule_name: self.id().to_string(),
                            description: "Function performs storage operations but lacks Env parameter".to_string(),
                            suggestion: "Add `env: &Env` parameter to function signature".to_string(),
                            line_number: i + 1,
                            column_number: 0,
                            variable_name: file_path.to_string(),
                            severity: self.severity(),
    ...
    fn check(&self, source: &str, file_path: &str) -> Option<Vec<RuleViolation>> {
        ...
        if source.contains("Instance") && !source.contains("new(") {
            violations.push(RuleViolation {
                ...
                suggestion: "Initialize Instance storage with `Instance::new(&env, key)`".to_string(),
                line_number: 1,
                column_number: 0,
                variable_name: file_path.to_string(),
