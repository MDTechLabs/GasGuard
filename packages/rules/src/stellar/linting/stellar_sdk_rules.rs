...
impl SorobanLintRule for SdkUsageRule {
    ...
    fn check(&self, source: &str, file_path: &str) -> Option<Vec<RuleViolation>> {
        ...
        if source.contains("Address") && !source.contains("require_auth") && !source.contains("require_auth_for_args") {
            violations.push(RuleViolation {
                rule_name: self.id().to_string(),
                description: "Contract uses Address type without authorization checks".to_string(),
                suggestion: "Add require_auth() or require_auth_for_args() for Address parameters".to_string(),
                line_number: 1,
                column_number: 0,
    ...
    fn check(&self, source: &str, file_path: &str) -> Option<Vec<RuleViolation>> {
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            if line.contains("Address") && (line.contains("pub fn") || line.contains("fn ")) {
                ...
                let next_lines = lines.iter().skip(i).take(15).copied().collect::<Vec<&str>>().join("\n");
                ...
                if !next_lines.contains("require_auth") && 
                   ...
                   !next_lines.contains("try_from_xdr") {
                    violations.push(RuleViolation {
                        rule_name: self.id().to_string(),
                        description: "Function with Address parameter lacks authorization validation".to_string(),
                        suggestion: "Add require_auth() or require_auth_for_args() to validate caller".to_string(),
                        line_number: i + 1,
                        column_number: 0,
                        variable_name: file_path.to_string(),
